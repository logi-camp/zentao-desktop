...
export default class Zentao {
    ...
    try {
      ...
      if (typeof remoteData === 'object' && remoteData !== null) {
        ...
        result = {
          ...
          md5: remoteData.md5,
          data: remoteData.data ?? remoteData.result,
        };
      } else {
      ...
      if (options.fields && typeof result.data === 'object' && result.data) {
        if (Array.isArray(result.data)) {
          ...
          result.data = slimmingObject(result.data, options.fields!);
        }
      }
  ...
  protected _log(
    ...
    const success = result ? result.status === 'success' : status === 200;
    const url = (resp ? resp.config.url : atrributes.url) ?? '';
    const method = (resp ? resp.config.method : atrributes.method) ?? 'GET';

    logLines.push(
      kleur.bold(
        `${kleur.gray('➡︎')} ${kleur[success ? 'green' : 'red']().inverse(` ${name} ${success ? '✓' : '𐄂'} `)}`
      )
    );

    logLines.push(`\n  ${kleur.bold().blue(method.toUpperCase())} ${kleur.blue().underline(url)}`);
    logLines.push(
      `    status: ${kleur[status === 200 ? 'green' : 'red'](`● ${status}`)} ${kleur.gray(resp?.statusText ?? '')}`
    );
    ...
    if (result) {
      ...
      logLines.push(`    status: ${kleur[result.status === 'success' ? 'green' : 'red'](result.status)},`);
      /* if (result.msg !== undefined) {
        logLines.push(
          `    msg: ${kleur[result.status === 'success' ? 'green' : 'red'](
            typeof result.msg === 'string' ? result.msg : JSON.stringify(result.msg)
          )},`
        );
      } */
      if (result.data !== undefined) {
        logLines.push(`    result: ${kleur.magenta(JSON.stringify(result.data))}`);
      }
    }
    if (resp && (!result || !success || !result.data)) {
